...
class PalindromePartitioning {
	...
	for (int k = i; k <= j - 1; k++) {
	    ...
	    int count = 1 + minPalinPartition(X, i, k) + minPalinPartition(X, k + 1, j);
    ...
    // Recursive function to find the minimum cuts needed in a String
    // such that each partition is a palindrome
    public static int minPalinPartition(int i, int j, boolean[][] isPalin, Map<String, Integer> lookup) {
	// base case: if starting index i and ending index j are equal
	// or X[i..j] is already a palindrome.
	...
	if (!lookup.containsKey(key)) {
	    ...
	    /*
	     * (X[i]) | (X[i+1..j]) (X[i..i+1]) | (X[i+2..j]) (X[i..i+2]) | (X[i+3..j]) ...
	     * ... (X[i..j-1]) | (X[j])
	     */
		...
		int count = 1 + minPalinPartition(i, k, isPalin, lookup) + minPalinPartition(k + 1, j, isPalin, lookup);
	...
	System.out.print("The minimum cuts required are " + minPalinPartition(0, X.length() - 1, isPalin, lookup));
	...
	System.out.println("The minimum cuts required are " + minPalinPartition(X, isPalin));
    }
}
